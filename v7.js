...
async function loadOlderMessages(){
  if(!state.activeTextChannelId)return;const rows=[...state.messageMap.values()].filter(m=>m.channel_id===state.activeTextChannelId);const earliest=rows.sort((a,b)=>new Date(a.created_at).getTime()-new Date(b.created_at).getTime())[0];if(!earliest)return;
...
// -----------------------------------------------------------------------------
ui.mobileBottomNav?.addEventListener("click",async(event)=>{const btn=event.target.closest("button[data-mobile-action]");if(!btn)return;const action=btn.dataset.mobileAction;if(action==="call"&&!state.voiceJoinedChannelId){toast("Entre em um canal de voz primeiro.");return;}ui.mobileBottomNav.querySelectorAll("button").forEach(b=>b.classList.toggle("active",b===btn));if(action==="servers")ui.mobileMenuBtn?.click();if(action==="chat"){rt.closeDrawers?.();if(state.activeTextChannelId)await rt.selectTextChannel(state.activeTextChannelId);}if(action==="call"){if(window.matchMedia("(max-width: 850px)").matches){if(!ui.mediaStage?.classList.contains("fullpage"))ui.fullpageMediaBtn?.click();}else{rt.toggleMediaExpanded?.(true);}ui.mediaStage?.classList.remove("hidden");}if(action==="friends")ui.friendsBtn?.click();if(action==="profile")ui.profileButton?.click();});
function syncMobileCallNavState(){
  if(!ui.mobileBottomNav)return;
  const callBtn=ui.mobileBottomNav.querySelector('[data-mobile-action="call"]');
  const chatBtn=ui.mobileBottomNav.querySelector('[data-mobile-action="chat"]');
  if(callBtn?.classList.contains('active')&&!ui.mediaStage?.classList.contains('fullpage')){
    callBtn.classList.remove('active');
    chatBtn?.classList.add('active');
  }
}
setInterval(syncMobileCallNavState,500);

// -----------------------------------------------------------------------------
// Dashboard inicial V7.1 — atividade e atalhos
// -----------------------------------------------------------------------------
function activeVoiceRooms() {
  return (state.channels || []).filter((c)=>c.type==="voice").map((c)=>{
    const unique=new Map();
    for(const person of state.voicePresence.get(c.id)||[]){const key=person?.user_id||person?.voice_client_id;if(key&&!unique.has(key))unique.set(key,person);}
    return {channel:c,people:[...unique.values()]};
...
async function checkUpcomingEvents() {
  ...
  try {
    ...
    for (const event of data || []) {
      ...
      const mins=Math.max(0,Math.round((new Date(event.starts_at).getTime()-now.getTime())/60000));
...
quickStreamQuality?.addEventListener("change", async () => {
  const quality=quickStreamQuality.value;
  const patch={ screen_quality: quality, low_bandwidth: false };
  if(quality==="1440p"&&Number(state.preferences.screen_fps||30)>30){patch.screen_fps=30;if(quickStreamFps)quickStreamFps.value="30";if(ui.screenFpsInput)ui.screenFpsInput.value="30";}
  await persistQuickPreference(patch);
  if (ui.screenQualityInput) ui.screenQualityInput.value = quality;
  if (ui.lowBandwidthInput) ui.lowBandwidthInput.checked = false;
  if (state.screenTrack) await applyScreenShareSettings?.();
  else toast(`${quality}${patch.screen_fps?" • 30 FPS":""} será usado na próxima transmissão.`, 1800);
});
quickStreamFps?.addEventListener("change", async () => {
  let value = Number(quickStreamFps.value || 30);
  if((state.preferences.screen_quality||"1080p")==="1440p"&&value>30){value=30;quickStreamFps.value="30";toast("1440p foi limitado a 30 FPS para manter estabilidade.",2200);}
...
let unreadRealtimeUserId = null;
let unreadRealtimeBusy = false;
...
async function ensureUnreadRealtime(){
  if(unreadRealtimeBusy)return;
  const userId=state.user?.id||null;
  if(!state.supabase||!userId){
    if(unreadRealtime){try{await state.supabase?.removeChannel(unreadRealtime);}catch(_){} unreadRealtime=null; unreadRealtimeUserId=null;}
    return;
  }
  if(unreadRealtime&&unreadRealtimeUserId===userId)return;
  unreadRealtimeBusy=true;
  try{
    if(unreadRealtime){try{await state.supabase.removeChannel(unreadRealtime);}catch(_){} unreadRealtime=null; unreadRealtimeUserId=null;}
    const ch=state.supabase.channel(`unread-global:${userId}`).on("postgres_changes",{event:"INSERT",schema:"public",table:"channel_messages"},({new:row})=>bumpChannelUnread(row?.channel_id,row));
    ch.subscribe(); unreadRealtime=ch; unreadRealtimeUserId=userId;
  }finally{unreadRealtimeBusy=false;}
}
...
let pwaReloading=false;updateNow?.addEventListener('click',()=>{if(waitingWorker)waitingWorker.postMessage({type:'SKIP_WAITING'});else location.reload();});updateLater?.addEventListener('click',hideUpdateBanner);navigator.serviceWorker?.addEventListener('controllerchange',()=>{if(pwaReloading)return;pwaReloading=true;location.reload();});setTimeout(watchForUpdates,2200);setInterval(async()=>{try{const reg=await navigator.serviceWorker?.getRegistration();await reg?.update();}catch(_){}},15*60*1000);

// -----------------------------------------------------------------------------
// V7.1 — Mobile: atalho persistente para voltar à call
// -----------------------------------------------------------------------------
const mobileCallPill=makeEl('button','mobile-call-pill hidden');mobileCallPill.type='button';mobileCallPill.innerHTML='<span class="mobile-call-pulse"></span><strong>Call ativa</strong><small>Toque para voltar</small>';document.body.appendChild(mobileCallPill);mobileCallPill.addEventListener('click',()=>{ui.mediaStage?.classList.remove('hidden','collapsed');if(window.matchMedia('(max-width: 850px)').matches&&!ui.mediaStage?.classList.contains('fullpage'))ui.fullpageMediaBtn?.click();else rt.toggleMediaExpanded?.(true);});
setInterval(()=>{const on=Boolean(state.voiceJoinedChannelId);const stageUnavailable=!ui.mediaStage||ui.mediaStage.classList.contains('hidden')||ui.mediaStage.classList.contains('collapsed');const show=on&&stageUnavailable&&!document.body.classList.contains('screen-full-viewport-mode')&&!document.body.classList.contains('call-fullpage-mode');mobileCallPill.classList.toggle('hidden',!show);document.body.classList.toggle('mobile-call-active',show);if(on){mobileCallPill.querySelector('strong').textContent=state.voiceJoinedChannelName||'Call ativa';}},700);
