...
async function leaveVoice() {
  ...
  ui.mediaGrid.replaceChildren(); ui.screenMediaGrid?.replaceChildren(); clearFocusedMediaCard(); refreshMediaSections(); ui.mediaStage.classList.add("hidden"); ui.mediaStage.classList.remove("collapsed", "expanded", "has-screen-share"); if (ui.expandMediaBtn) { ui.expandMediaBtn.textContent = "⛶"; ui.expandMediaBtn.title = "Ampliar área de mídia"; }
...
function focusMediaCard(card, force = null) {
  ...
  if (video?.srcObject) {
    ...
    requestAnimationFrame(() => { video.srcObject = stream; video.play?.().catch(() => {}); });
  }
}
...
function toggleMediaExpanded(force = null) {
  ...
  if (shouldExpand) ui.mediaStage.classList.remove("fullpage");
...
async function boot() {
  ...
  if ("serviceWorker" in navigator && location.protocol === "https:") navigator.serviceWorker.register("./sw.js?v=7.3.7").catch(() => {});
