...
const UI_SVG = {
  focus: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 4H4v4"></path><path d="M16 4h4v4"></path><path d="M20 16v4h-4"></path><path d="M4 16v4h4"></path></svg>',
  floating: '<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="4" y="6" width="12" height="10" rx="2"></rect><path d="M10 18h10a2 2 0 0 0 2-2V8a2 2 0 0 0-2-2h-2"></path></svg>',
  highlight: '<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="4" y="5" width="16" height="14" rx="2"></rect><path d="M9 9h6M9 12h6M9 15h4"></path></svg>',
  mic: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3a3 3 0 0 0-3 3v6a3 3 0 0 0 6 0V6a3 3 0 0 0-3-3Z"></path><path d="M5 11a7 7 0 0 0 14 0M12 18v3M9 21h6"></path></svg>',
  headphones: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 14v-2a8 8 0 0 1 16 0v2"></path><path d="M4 14h3v6H5a1 1 0 0 1-1-1v-5ZM20 14h-3v6h2a1 1 0 0 0 1-1v-5Z"></path></svg>',
  camera: '<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="6" width="13" height="12" rx="2"></rect><path d="m16 10 5-3v10l-5-3Z"></path></svg>',
  screen: '<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="4" width="18" height="13" rx="2"></rect><path d="M8 21h8M12 17v4"></path></svg>',
  hangup: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 15c5-3 11-3 16 0"></path><path d="m6 14-2 4M18 14l2 4"></path></svg>',
  screenfill: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 3H3v5M16 3h5v5M21 16v5h-5M3 16v5h5"></path></svg>',
  screenexit: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 8H3V3M16 8h5V3M21 16h-5v5M3 16h5v5"></path></svg>'
};
function iconMarkup(name){ return UI_SVG[name] || ''; }
function setSvgButton(button, name){ if (!button || !UI_SVG[name]) return; button.innerHTML = UI_SVG[name]; button.classList.add('svg-icon-button'); }
function profilePresenceText(profile = state.profile) {
  const mode = profile?.presence_mode || "online";
  return mode === "dnd" ? "Não perturbe" : mode === "idle" ? "Ausente" : mode === "invisible" ? "Invisível" : "Disponível";
}
function updatePrefsSidebarSummary() {
  const avatar = document.getElementById('prefsSidebarAvatar');
  const name = document.getElementById('prefsSidebarName');
  const status = document.getElementById('prefsSidebarStatus');
  const summary = document.getElementById('prefsSummaryUser');
  const displayName = state.profile?.display_name || state.profile?.username || 'Usuário';
  const handle = '@' + (state.profile?.username || displayName).toLowerCase();
  const presence = profilePresenceText(state.profile);
  const bio = state.profile?.bio?.trim() || 'Personalize seu perfil para deixar sua conta com a sua cara.';
  const customStatus = state.profile?.custom_status?.trim() || 'Sem status personalizado';
  if (avatar) setAvatar(avatar, state.profile || { display_name: displayName, username: displayName });
  if (name) name.textContent = displayName;
  if (status) status.textContent = handle;
  if (summary) summary.textContent = displayName;
  if (ui.prefsSummaryStatus) ui.prefsSummaryStatus.textContent = presence;
  if (ui.prefsAccountAvatar) setAvatar(ui.prefsAccountAvatar, state.profile || { display_name: displayName, username: displayName }, 'avatar-xl');
  if (ui.prefsAccountDisplayName) ui.prefsAccountDisplayName.textContent = displayName;
  if (ui.prefsAccountHandle) ui.prefsAccountHandle.textContent = handle;
  if (ui.prefsAccountBio) ui.prefsAccountBio.textContent = bio;
  if (ui.prefsAccountStatus) ui.prefsAccountStatus.textContent = presence;
  if (ui.prefsAccountCustomStatus) ui.prefsAccountCustomStatus.textContent = customStatus;
}
function activatePreferencesTab(tab) {
  const target = tab || 'account';
  document.querySelectorAll('[data-pref-tab]').forEach((btn) => btn.classList.toggle('active', btn.dataset.prefTab === target));
  document.querySelectorAll('[data-pref-pane]').forEach((pane) => pane.classList.toggle('hidden', pane.dataset.prefPane !== target));
}
...
const ui = {
  ...
  mediaStage: $("#mediaStage"), mediaStageTitle: $("#mediaStageTitle"), callStatusText: $("#callStatusText"), mediaGrid: $("#mediaGrid"), screenMediaSection: $("#screenMediaSection"), screenMediaGrid: $("#screenMediaGrid"), cameraMediaSection: $("#cameraMediaSection"), expandMediaBtn: $("#expandMediaBtn"), fullpageMediaBtn: $("#fullpageMediaBtn"), collapseMediaBtn: $("#collapseMediaBtn"), mediaStageControls: $("#mediaStageControls"), stageMuteBtn: $("#stageMuteBtn"), stageDeafenBtn: $("#stageDeafenBtn"), stageCameraBtn: $("#stageCameraBtn"), stageShareBtn: $("#stageShareBtn"), stageLeaveBtn: $("#stageLeaveBtn"),
  ...
  preferencesDialog: $("#preferencesDialog"), preferencesForm: $("#preferencesForm"), prefsEditProfileBtn: $("#prefsEditProfileBtn"), prefsAccountAvatar: $("#prefsAccountAvatar"), prefsAccountDisplayName: $("#prefsAccountDisplayName"), prefsAccountHandle: $("#prefsAccountHandle"), prefsAccountBio: $("#prefsAccountBio"), prefsAccountStatus: $("#prefsAccountStatus"), prefsAccountCustomStatus: $("#prefsAccountCustomStatus"), prefsSummaryStatus: $("#prefsSummaryStatus"), accentColorInput: $("#accentColorInput"), themeModeInput: $("#themeModeInput"), densityInput: $("#densityInput"), audioInputSelect: $("#audioInputSelect"), videoInputSelect: $("#videoInputSelect"), audioOutputPreference: $("#audioOutputPreference"), screenQualityInput: $("#screenQualityInput"), screenFpsInput: $("#screenFpsInput"), streamModeInput: $("#streamModeInput"), notificationLevelInput: $("#notificationLevelInput"), pushToTalkInput: $("#pushToTalkInput"), pushToTalkKeyInput: $("#pushToTalkKeyInput"), voiceSensitivityInput: $("#voiceSensitivityInput"), voiceSensitivityValue: $("#voiceSensitivityValue"), lowBandwidthInput: $("#lowBandwidthInput"), refreshDevicesBtn: $("#refreshDevicesBtn"), testMicBtn: $("#testMicBtn"), micMeter: $("#micMeter"), micDeviceStatus: $("#micDeviceStatus"), openDiagnosticsFromPrefsBtn: $("#openDiagnosticsFromPrefsBtn"), installAppBtn: $("#installAppBtn"), checkUpdateBtn: $("#checkUpdateBtn"), exportDataBtn: $("#exportDataBtn"), signOutAllBtn: $("#signOutAllBtn"), reduceMotionInput: $("#reduceMotionInput"), browserNotificationsInput: $("#browserNotificationsInput"), soundsInput: $("#soundsInput"),
...
function dedupeVoicePresenceEntries(entries = [], { uniqueUsers = false } = {}) {
  const byKey = new Map();
  for (const raw of entries || []) {
    if (!raw?.voice_client_id) continue;
    const key = uniqueUsers ? String(raw.user_id || raw.voice_client_id) : String(raw.voice_client_id);
    const current = byKey.get(key);
    if (!current) { byKey.set(key, { ...raw }); continue; }

    if (uniqueUsers) {
      // Para uma mesma conta, mantemos a sessão mais antiga. Isso combina com o
      // bloqueio de duplicidade e evita alternar visualmente entre duas abas/dispositivos.
      const currentJoined = Date.parse(current.joined_at || 0) || Number.MAX_SAFE_INTEGER;
      const nextJoined = Date.parse(raw.joined_at || 0) || Number.MAX_SAFE_INTEGER;
      const currentWins = currentJoined !== nextJoined
        ? currentJoined < nextJoined
        : String(current.voice_client_id).localeCompare(String(raw.voice_client_id)) <= 0;
      byKey.set(key, { ...(currentWins ? current : raw) });
      continue;
    }

    // Para o mesmo cliente de voz, usamos a metadata mais recente. Não fazemos OR
    // dos flags porque isso poderia deixar câmera/tela/mudo presos em "ligado" após desligar.
    const currentUpdated = Date.parse(current.presence_updated_at || current.joined_at || 0) || 0;
    const nextUpdated = Date.parse(raw.presence_updated_at || raw.joined_at || 0) || 0;
    byKey.set(key, { ...(nextUpdated >= currentUpdated ? raw : current) });
  }
  return [...byKey.values()].sort((a, b) => (Date.parse(a.joined_at || 0) || 0) - (Date.parse(b.joined_at || 0) || 0));
}
function getVoicePresenceList(channelId, options = {}) {
  return dedupeVoicePresenceEntries(state.voicePresence.get(channelId) || [], options);
}
function findExistingVoiceSession(userId, excludeClientId = state.voiceClientId) {
  if (!userId) return null;
  for (const channel of state.channels.filter((item) => item.type === "voice")) {
    const entries = getVoicePresenceList(channel.id);
    const hit = entries.find((entry) => entry.user_id === userId && entry.voice_client_id !== excludeClientId);
    if (hit) return { channel, entry: hit };
  }
  return null;
}
async function resolveDuplicateCurrentVoiceSession() {
  if (state.duplicateVoiceGuardBusy || !state.voiceJoinedChannelId || !state.user?.id) return;
  const localJoinedAt = Date.parse(state.voiceJoinedAt || 0) || Date.now();
  const contenders = [{ voice_client_id: state.voiceClientId, joined_at: state.voiceJoinedAt || new Date(localJoinedAt).toISOString(), local: true }];
  for (const channel of state.channels.filter((item) => item.type === "voice")) {
    for (const entry of getVoicePresenceList(channel.id)) {
      if (entry.user_id !== state.user.id || entry.voice_client_id === state.voiceClientId) continue;
      contenders.push({ ...entry, channel_id: channel.id, local: false });
    }
  }
  ...
  contenders.sort((a, b) => {
    ...
    return String(a.voice_client_id).localeCompare(String(b.voice_client_id));
  });
  if (contenders[0].voice_client_id === state.voiceClientId) return;
  state.duplicateVoiceGuardBusy = true;
  try {
    await leaveVoice();
    toast("Esta conta já estava conectada em outra call. Mantivemos apenas a conexão mais antiga.", 5200);
  } finally {
    state.duplicateVoiceGuardBusy = false;
  }
}
...
async function loadPreferences() {
  ...
  if (state.preferences.screen_quality === "1440p" && Number(state.preferences.screen_fps || 30) > 30) {
    state.preferences.screen_fps = 30;
    safeLocalStorageSet("tropa_preferences", JSON.stringify(state.preferences));
    if (state.supabase && state.user) {
      try { await state.supabase.from("user_preferences").update({ screen_fps: 30 }).eq("user_id", state.user.id); } catch (_) {}
    }
  }
  applyPreferences();
}
...
async function openPreferences() {
  ...
  updatePrefsSidebarSummary();
  activatePreferencesTab("account");
  const prefsSearch = document.getElementById("preferencesSearchInput");
  if (prefsSearch) prefsSearch.value = "";
  document.querySelectorAll("[data-pref-tab]").forEach((btn) => btn.classList.remove("hidden"));
  document.getElementById("advancedInstallAppBtn")?.classList.toggle("hidden", document.getElementById("installAppBtn")?.classList.contains("hidden") !== false);
}
...
async function savePreferences() {
  ...
  if (next.screen_quality === "1440p" && next.screen_fps > 30) {
    next.screen_fps = 30;
    if (ui.screenFpsInput) ui.screenFpsInput.value = "30";
  }
...
document.querySelectorAll("[data-pref-tab]").forEach((btn) => btn.addEventListener("click", () => activatePreferencesTab(btn.dataset.prefTab)));
document.querySelectorAll(".mirror-action").forEach((btn) => btn.addEventListener("click", () => document.getElementById(btn.dataset.mirrorTarget)?.click()));
document.getElementById("preferencesSearchInput")?.addEventListener("input", (event) => {
  const term = String(event.target.value || "").trim().toLowerCase();
  [...document.querySelectorAll("[data-pref-tab]")].forEach((btn) => btn.classList.toggle("hidden", Boolean(term) && !btn.textContent.toLowerCase().includes(term)));
});
...
window.addEventListener("keydown", (event) => {
  if (event.key === "Escape") {
    const wasScreenFill = document.body.classList.contains("screen-full-viewport-mode");
    if (wasScreenFill) clearScreenFillMode();
    else {
      clearFocusedMediaCard();
      if (ui.mediaStage?.classList.contains("fullpage")) toggleMediaFullpage(false);
    }
    ui.serverQuickMenu.classList.add("hidden");
    closeDrawers();
...
function updateOwnProfileUI() {
  ...
  const presenceText = profilePresenceText(p);
...
ui.prefsEditProfileBtn?.addEventListener("click", () => {
  closeDialog(ui.preferencesDialog);
  setTimeout(() => openProfileEditor(), 80);
});
...
async function openDm(userId) {
  ...
  if (token !== state.dmLoadToken || state.activeDmUserId !== userId) return;
...
function closeDm() { ++state.dmLoadToken; state.activeDmUserId = null; ui.dmConversation.classList.add("hidden"); ui.dmEmpty.classList.remove("hidden"); ui.dmMessages.replaceChildren(); }
...
async function selectServer(serverId, initial = false) {
  ...
  try {
    ...
    if (token !== state.serverLoadToken || state.activeServerId !== serverId) return;
...
async function loadServerBundle() {
  ...
  if (ids.length) {
    ...
    if (error) throw error;
    for (const profile of data || []) profilesMap.set(profile.user_id, profile);
  }

  // A troca de servidor pode acontecer enquanto as consultas estão em voo.
  // Só aplicamos o pacote se ele ainda pertencer ao servidor atualmente aberto.
  if (sid !== state.activeServerId) return false;
...
async function setupServerPresence() {
  ...
  try {
    ...
    try { await state.supabase.removeChannel(ch); } catch (_) {}
    if (state.serverPresence === ch) state.serverPresence = null;
    throw error;
  }
}
...
function renderChannels() {
  ...
  const makeRow = (channel) => {
    ...
    if (channel.type === "text") {
      ...
      const voiceCount = getVoicePresenceList(channel.id, { uniqueUsers: true }).length;
    ...
    if (channel.type === "voice") {
      ...
      for (const p of getVoicePresenceList(channel.id, { uniqueUsers: true })) {
...
ui.messageForm.addEventListener("submit", async (event) => {
  ...
  if (Number(currentChannel?.slowmode_seconds || 0) > 0 && !hasPermission("MANAGE_MESSAGES")) {
    const mine = [...state.messageMap.values()].filter((m) => m.channel_id === state.activeTextChannelId && m.user_id === state.user.id).sort((a,b) => new Date(b.created_at).getTime()-new Date(a.created_at).getTime())[0];
    if (mine) {
...
async function ensureVoiceWatcher(channel) {
  const existing = state.voiceWatchers.get(channel.id);
  if (existing && !["CHANNEL_ERROR", "TIMED_OUT", "CLOSED"].includes(existing.status)) return existing;
  if (existing) {
    state.voiceWatchers.delete(channel.id);
    try { await state.supabase.removeChannel(existing.realtime); } catch (_) {}
  }
  ...
  const sync = () => {
    ...
    state.voicePresence.set(channel.id, dedupeVoicePresenceEntries(list));
    renderChannels(); renderMembers();
    if (channel.id === state.voiceJoinedChannelId) syncVoicePeers();
    if (state.voiceJoinedChannelId) resolveDuplicateCurrentVoiceSession().catch((error) => console.warn("Duplicate voice guard", error));
  };
...
function voicePresencePayload() {
  return {
    ...
    joined_at: state.voiceJoinedAt || new Date().toISOString(),
...
async function retrackVoicePresence() {
  ...
  let watcher = state.voiceWatchers.get(state.voiceJoinedChannelId);
  try {
    if (!watcher || ["CHANNEL_ERROR", "TIMED_OUT", "CLOSED"].includes(watcher.status)) {
      const channel = state.channels.find((item) => item.id === state.voiceJoinedChannelId && item.type === "voice");
      if (!channel) return;
      watcher = await ensureVoiceWatcher(channel);
      await Promise.race([watcher.ready, sleep(8000).then(() => { throw new Error("Timeout ao restaurar presença de voz"); })]);
    }
    await watcher.realtime.track(voicePresencePayload());
  } catch (error) {
    console.warn("Falha ao atualizar presença da call", error);
    return;
  }
...
async function joinVoice(channelId) {
  ...
  if (state.voiceJoinedChannelId) await leaveVoice();
  const existingSession = findExistingVoiceSession(state.user?.id, state.voiceClientId);
  if (existingSession) {
    const where = existingSession.channel?.name || existingSession.entry?.display_name || "outra call";
    toast(`Esta conta já está conectada em 🔊 ${where}. Saia da outra call antes de entrar novamente.`, 5200);
    return;
  }
  ...
  try {
    ...
    state.localStream = stream; state.voiceJoinedChannelId = channel.id; state.voiceJoinedChannelName = channel.name; state.voiceJoinedAt = new Date().toISOString(); state.hardMuted = false; if (state.preferences.push_to_talk && micTrack()) micTrack().enabled = false;
    ...
    const duplicateSession = findExistingVoiceSession(state.user?.id, state.voiceClientId);
    if (duplicateSession) throw new Error("USER_ALREADY_IN_VOICE");
    await watcher.realtime.track(voicePresencePayload());
    ...
    if (session === state.voiceSession) {
      stream?.getTracks().forEach((t) => t.stop()); state.localStream = null; state.voiceJoinedChannelId = null; state.voiceJoinedChannelName = null; state.voiceJoinedAt = null; state.joiningVoice = false; updateVoiceDock("idle");
      const duplicate = String(error?.message || "") === "USER_ALREADY_IN_VOICE";
      toast(duplicate ? "Esta conta já está em uma call em outra aba ou dispositivo." : (error?.name === "NotAllowedError" ? "Permita o acesso ao microfone para entrar na chamada." : "Não foi possível entrar na chamada."), 5500);
    }
  }
}
...
async function leaveVoice() {
  ...
  clearScreenFillMode({ restore: false });
  ...
  await stopScreenAudioMix({ restoreMic: false, stopCapturedTrack: false });
  state.voiceJoinedChannelId = null; state.voiceJoinedChannelName = null; state.voiceJoinedAt = null; state.localStream = null; state.cameraTrack = null; state.screenTrack = null; state.screenAudioTrack = null; state.deafen = false; state.hardMuted = false; state.cameraBusy = false; state.screenBusy = false;
  ...
  ui.mediaGrid.replaceChildren(); ui.screenMediaGrid?.replaceChildren(); clearFocusedMediaCard(); refreshMediaSections(); ui.mediaStage.classList.add("hidden"); ui.mediaStage.classList.remove("collapsed", "expanded", "fullpage", "focus-mode", "has-screen-share");
  document.body.classList.remove("call-fullpage-mode");
  syncMediaStageButtons();
...
function updateVoiceDock(mode, channel = null) {
  ...
  ui.stageMuteBtn?.classList.toggle("active", mutedNow);
  ui.stageDeafenBtn?.classList.toggle("active", state.deafen);
  ui.stageCameraBtn?.classList.toggle("active", Boolean(state.cameraTrack));
  ui.stageShareBtn?.classList.toggle("active", Boolean(state.screenTrack));
  if (ui.stageMuteBtn) ui.stageMuteBtn.disabled = !joined || state.cameraBusy || state.screenBusy;
  if (ui.stageDeafenBtn) ui.stageDeafenBtn.disabled = !joined;
  if (ui.stageCameraBtn) ui.stageCameraBtn.disabled = !joined || state.cameraBusy || state.screenBusy || Boolean(state.screenTrack);
  if (ui.stageShareBtn) ui.stageShareBtn.disabled = !joined || state.cameraBusy || state.screenBusy || !screenShareSupported();
  if (ui.stageLeaveBtn) ui.stageLeaveBtn.disabled = !joined && !connecting;
}
...
setSvgButton(ui.expandMediaBtn, "floating");
setSvgButton(ui.fullpageMediaBtn, "focus");
setSvgButton(ui.stageMuteBtn, "mic");
setSvgButton(ui.stageDeafenBtn, "headphones");
setSvgButton(ui.stageCameraBtn, "camera");
setSvgButton(ui.stageShareBtn, "screen");
setSvgButton(ui.stageLeaveBtn, "hangup");
ui.expandMediaBtn?.addEventListener("click", () => toggleMediaExpanded());
ui.fullpageMediaBtn?.addEventListener("click", () => toggleMediaFullpage());
ui.collapseMediaBtn.addEventListener("click", () => {
  if (document.body.classList.contains("screen-full-viewport-mode")) clearScreenFillMode({ restore: false });
  if (ui.mediaStage.classList.contains("fullpage")) {
    ui.mediaStage.classList.remove("fullpage", "expanded");
    document.body.classList.remove("call-fullpage-mode");
    ui.mediaStage.classList.add("collapsed");
  } else ui.mediaStage.classList.toggle("collapsed");
  ui.collapseMediaBtn.textContent = ui.mediaStage.classList.contains("collapsed") ? "□" : "—";
  syncMediaStageButtons();
});
ui.stageMuteBtn?.addEventListener("click", () => ui.muteBtn?.click());
ui.stageDeafenBtn?.addEventListener("click", () => ui.deafenBtn?.click());
ui.stageCameraBtn?.addEventListener("click", () => ui.cameraBtn?.click());
ui.stageShareBtn?.addEventListener("click", () => ui.shareBtn?.click());
ui.stageLeaveBtn?.addEventListener("click", () => ui.leaveVoiceBtn?.click());
syncMediaStageButtons();
...
function clearFocusedMediaCard() {
  ui.mediaStage?.classList.remove("focus-mode");
  document.querySelectorAll(".media-card.media-card-focus").forEach((el) => el.classList.remove("media-card-focus"));
  removeFullscreenSelfView();
}
function focusMediaCard(card, force = null) {
  if (!card) return;
  const already = card.classList.contains("media-card-focus");
  const shouldFocus = force === null ? !already : Boolean(force);
  clearFocusedMediaCard();
  if (!shouldFocus) return;
  ui.mediaStage?.classList.remove("collapsed");
  ui.mediaStage?.classList.add("focus-mode");
  toggleMediaFullpage(true);
  card.classList.add("media-card-focus");
  addFullscreenSelfView(card);
  card.querySelector("video")?.play?.().catch(() => {});
}
function toggleCardFullscreen(card) { focusMediaCard(card); }
function mediaGeometryFromVideo(video, fallbackTrack = null) {
...
function applyMediaCardGeometry(card, video, { screen = false, track = null } = {}) {
  if (!card || !video) return;
  const geometry = mediaGeometryFromVideo(video, track);
  if (!geometry) return;
  const aspect = Math.max(.2, Math.min(6, geometry.aspect));
  card.style.setProperty(screen ? "--stream-aspect" : "--camera-aspect", String(aspect));
  card.dataset.mediaWidth = String(geometry.width);
  card.dataset.mediaHeight = String(geometry.height);
  card.dataset.mediaOrientation = aspect > 1.18 ? "landscape" : aspect < .85 ? "portrait" : "square";
}
function bindMediaGeometry(card, video, options = {}) {
  if (!card || !video) return;
  const sync = () => {
    const currentTrack = video.srcObject?.getVideoTracks?.()[0] || options.track || null;
    applyMediaCardGeometry(card, video, { screen: card.classList.contains("screen") || Boolean(options.screen), track: currentTrack });
  };
  if (video.dataset.geometryBound !== "1") {
    video.dataset.geometryBound = "1";
...
let screenFillRestoreState = null;
function syncScreenFillButton(button, active) {
  if (!button) return;
  button.innerHTML = iconMarkup(active ? "screenexit" : "screenfill");
  button.classList.add("svg-icon-button", "screen-fill-btn");
  button.classList.toggle("active", Boolean(active));
  button.setAttribute("aria-pressed", active ? "true" : "false");
  button.title = active ? "Sair da tela cheia da transmissão" : "Preencher a tela toda";
  button.setAttribute("aria-label", button.title);
}
function clearScreenFillMode({ restore = true } = {}) {
  const activeCard = document.querySelector(".media-card.screen-full-target");
  activeCard?.classList.remove("screen-full-target");
  ui.mediaStage?.classList.remove("screen-full-viewport");
  document.body.classList.remove("screen-full-viewport-mode");
  document.querySelectorAll(".screen-fill-btn").forEach((button) => syncScreenFillButton(button, false));
  const previous = screenFillRestoreState;
  screenFillRestoreState = null;
  if (restore && previous && state.voiceJoinedChannelId && ui.mediaStage) {
    ui.mediaStage.classList.toggle("fullpage", Boolean(previous.fullpage));
    ui.mediaStage.classList.toggle("expanded", Boolean(previous.expanded) && !previous.fullpage);
    document.body.classList.toggle("call-fullpage-mode", Boolean(previous.fullpage));
    syncMediaStageButtons();
  }
}
function toggleScreenFillMode(card, force = null) {
  if (!card || !card.classList.contains("screen")) return;
  const already = card.classList.contains("screen-full-target");
  const shouldFill = force === null ? !already : Boolean(force);
  if (!shouldFill) { clearScreenFillMode(); return; }
  if (!document.body.classList.contains("screen-full-viewport-mode")) {
    screenFillRestoreState = {
      fullpage: Boolean(ui.mediaStage?.classList.contains("fullpage")),
      expanded: Boolean(ui.mediaStage?.classList.contains("expanded"))
    };
  }
  clearFocusedMediaCard();
  ui.mediaStage?.classList.remove("collapsed", "expanded", "fullpage", "focus-mode");
  document.body.classList.remove("call-fullpage-mode");
  ui.mediaStage?.classList.add("screen-full-viewport");
  card.classList.add("screen-full-target");
  document.body.classList.add("screen-full-viewport-mode");
  syncScreenFillButton(card.querySelector(".screen-fill-btn"), true);
  const fillVideo = card.querySelector("video");
  const fillTrack = fillVideo?.srcObject?.getVideoTracks?.()[0] || null;
  applyMediaCardGeometry(card, fillVideo, { screen: true, track: fillTrack });
  fillVideo?.play?.().then(() => applyMediaCardGeometry(card, fillVideo, { screen: true, track: fillTrack })).catch(() => {});
}
document.addEventListener("keydown", (event) => {
  if (event.key !== "Escape") return;
  if (document.body.classList.contains("screen-full-viewport-mode")) { clearScreenFillMode(); return; }
  if (ui.mediaStage?.classList.contains("focus-mode")) clearFocusedMediaCard();
});

function ensureLocalScreenCard() {
  let card = document.getElementById("media-local-screen");
  if (!state.screenTrack) {
    if (card?.classList.contains("screen-full-target")) clearScreenFillMode();
  ...
  if (!card) {
    ...
    const expandBtn = makeEl("button", "tiny-icon media-expand-btn"); expandBtn.type = "button"; expandBtn.title = "Destacar transmissão"; expandBtn.setAttribute("aria-label", "Destacar transmissão"); expandBtn.innerHTML = iconMarkup("highlight"); expandBtn.classList.add("svg-icon-button");
    expandBtn.addEventListener("click", (event) => { event.stopPropagation(); toggleCardFullscreen(card); });
    const fillBtn = makeEl("button", "tiny-icon screen-fill-btn"); fillBtn.type = "button"; syncScreenFillButton(fillBtn, false);
    fillBtn.addEventListener("click", (event) => { event.stopPropagation(); toggleScreenFillMode(card); });
    controls.append(expandBtn, fillBtn);
  ...
  bindMediaGeometry(card, video, { screen: true, track: state.screenTrack });
  applyMediaCardGeometry(card, video, { screen: true, track: state.screenTrack });
  video.play().then(() => applyMediaCardGeometry(card, video, { screen: true, track: state.screenTrack })).catch(() => {});
...
function ensureLocalCard() {
  ...
  if (!card) {
    ...
    const controls = makeEl("div", "media-local-controls");
    const expandBtn = makeEl("button", "tiny-icon media-expand-btn"); expandBtn.type = "button"; expandBtn.title = "Destacar câmera"; expandBtn.setAttribute("aria-label", "Destacar câmera"); expandBtn.innerHTML = iconMarkup("highlight"); expandBtn.classList.add("svg-icon-button");
    expandBtn.addEventListener("click", (event) => { event.stopPropagation(); toggleCardFullscreen(card); });
  ...
  if (cameraTrack) {
    ...
    bindMediaGeometry(card, video, { screen: false, track: cameraTrack });
    applyMediaCardGeometry(card, video, { screen: false, track: cameraTrack });
    video.play().then(() => applyMediaCardGeometry(card, video, { screen: false, track: cameraTrack })).catch(() => {});
  } else {
...
function syncMediaStageButtons() {
  const expanded = ui.mediaStage?.classList.contains("expanded");
  const fullpage = ui.mediaStage?.classList.contains("fullpage");
  const collapsed = ui.mediaStage?.classList.contains("collapsed");
  if (ui.collapseMediaBtn) {
    ui.collapseMediaBtn.textContent = collapsed ? "□" : "—";
    ui.collapseMediaBtn.title = collapsed ? "Restaurar call" : "Minimizar call";
    ui.collapseMediaBtn.setAttribute("aria-label", ui.collapseMediaBtn.title);
  }
  if (ui.expandMediaBtn) {
    ui.expandMediaBtn.classList.toggle("active", Boolean(expanded));
    ui.expandMediaBtn.setAttribute("aria-pressed", expanded ? "true" : "false");
    ui.expandMediaBtn.title = expanded ? "Sair do modo flutuante" : "Modo flutuante";
  }
  if (ui.fullpageMediaBtn) {
    ui.fullpageMediaBtn.classList.toggle("active", Boolean(fullpage));
    ui.fullpageMediaBtn.setAttribute("aria-pressed", fullpage ? "true" : "false");
    ui.fullpageMediaBtn.title = fullpage ? "Sair do modo tela cheia da call" : "Ocupar a tela toda";
  }
}
function toggleMediaExpanded(force = null) {
  ...
  if (shouldExpand) { ui.mediaStage.classList.remove("fullpage", "collapsed"); document.body.classList.remove("call-fullpage-mode"); }
  ui.mediaStage.classList.toggle("expanded", shouldExpand);
  if (!shouldExpand) clearFocusedMediaCard();
  syncMediaStageButtons();
}
function toggleMediaFullpage(force = null) {
  if (document.body.classList.contains("screen-full-viewport-mode")) clearScreenFillMode({ restore: false });
  const shouldFullpage = force === null ? !ui.mediaStage.classList.contains("fullpage") : Boolean(force);
  if (shouldFullpage) ui.mediaStage.classList.remove("expanded", "collapsed");
  ui.mediaStage.classList.toggle("fullpage", shouldFullpage);
  document.body.classList.toggle("call-fullpage-mode", shouldFullpage);
  if (!shouldFullpage && !ui.mediaStage.classList.contains("expanded")) clearFocusedMediaCard();
  syncMediaStageButtons();
}

function peerPresence(peerId) {
  const list = getVoicePresenceList(state.voiceJoinedChannelId, { uniqueUsers: true });
...
function makeRemoteCard(peerId) {
  ...
  if (!card) {
    ...
    const mediaAvatar = makeEl("span", "media-avatar avatar");
    const controls = makeEl("div", "media-local-controls");
    const expandBtn = makeEl("button", "tiny-icon media-expand-btn"); expandBtn.type = "button"; expandBtn.title = "Destacar participante"; expandBtn.setAttribute("aria-label", "Destacar participante"); expandBtn.innerHTML = iconMarkup("highlight"); expandBtn.classList.add("svg-icon-button");
    expandBtn.addEventListener("click", (event) => { event.stopPropagation(); toggleCardFullscreen(card); });
    const fillBtn = makeEl("button", "tiny-icon screen-fill-btn hidden"); fillBtn.type = "button"; syncScreenFillButton(fillBtn, false);
    fillBtn.addEventListener("click", (event) => { event.stopPropagation(); toggleScreenFillMode(card); });
    controls.append(expandBtn, fillBtn);
  ...
  const liveVideoTrack = peer.remoteStream.getVideoTracks().find((t) => t.readyState === "live" && !t.muted) || null;
  const liveVideo = Boolean(liveVideoTrack);
  const remoteScreen = Boolean(peer.remoteMedia?.screen);
  if (!remoteScreen && card.classList.contains("screen-full-target")) clearScreenFillMode();
  card.classList.toggle("audio-only", !liveVideo); card.classList.toggle("screen", remoteScreen);
  if (liveVideo) {
    bindMediaGeometry(card, video, { screen: remoteScreen, track: liveVideoTrack });
    applyMediaCardGeometry(card, video, { screen: remoteScreen, track: liveVideoTrack });
  }
  const fillBtn = card.querySelector(".screen-fill-btn");
  fillBtn?.classList.toggle("hidden", !remoteScreen);
  if (fillBtn && !card.classList.contains("screen-full-target")) syncScreenFillButton(fillBtn, false);
...
function removeRemoteCard(peerId) { state.audioMeters.get(peerId)?.stop?.(); state.audioMeters.delete(peerId); const card = document.getElementById(`media-${peerId}`); if (card?.classList.contains("screen-full-target")) clearScreenFillMode(); card?.remove(); refreshMediaSections(); updateCallStatus(); }
...
function createPeer(peerId) {
  ...
  const presenceMedia = presence ? { camera: Boolean(presence.camera), screen: Boolean(presence.screen), muted: Boolean(presence.muted), deafened: Boolean(presence.deafened) } : null;
  const peer = { pc, remoteStream, audioSender: null, videoSender: null, makingOffer: false, ignoreOffer: false, isSettingRemoteAnswerPending: false, pendingCandidates: [], polite: state.voiceClientId.localeCompare(peerId) > 0, remoteMedia: state.remoteMediaStates.get(peerId) || presenceMedia || { camera: false, screen: false, muted: false, deafened: false }, restartTimer: null, connectTimer: null, restartCount: 0, warningShown: false, lastRestartAt: 0, lastRestartReason: "", candidateTypes: new Set(), iceErrors: [] };
...
function activeVoicePeers() {
  const list = getVoicePresenceList(state.voiceJoinedChannelId, { uniqueUsers: true }); const peers = [];
...
async function toggleCamera() {
  ...
  try {
    ...
    const baseConstraint = { width: { ideal: low ? 640 : 1280 }, height: { ideal: low ? 360 : 720 }, frameRate: { ideal: low ? 15 : 30, max: low ? 15 : 30 }, facingMode: "user" };
    let stream = null;
    try {
      const videoConstraint = { ...baseConstraint };
      ...
      if (state.preferences.video_input_id && ["OverconstrainedError", "NotFoundError", "DevicesNotFoundError", "NotReadableError", "AbortError"].includes(cameraError?.name)) {
        stream = await navigator.mediaDevices.getUserMedia({ video: baseConstraint, audio: false });
        state.preferences.video_input_id = null;
        safeLocalStorageSet("tropa_preferences", JSON.stringify(state.preferences));
        try { await state.supabase?.from("user_preferences").update({ video_input_id: null }).eq("user_id", state.user.id); } catch (_) {}
        toast("A câmera salva não está disponível. Usando a câmera padrão.", 4200);
...
function normalizeScreenProfileSettings() {
  let quality = state.preferences.screen_quality || "1080p";
  let fps = Math.max(5, Math.min(60, Number(state.preferences.screen_fps || 30)));
  const low = Boolean(state.preferences.low_bandwidth);
  if (quality === "1440p" && fps > 30) fps = 30;
  if (low) fps = Math.min(fps, 15);
  return { quality, fps, low };
}
function getScreenShareVideoConstraints() {
  // Capture the selected display at its native geometry. Width/height constraints on
  // getDisplayMedia can force 16:9 scaling/cropping on 16:10, ultrawide and portrait
  // screens. Resolution limiting is done later on RTCRtpSender, preserving aspect ratio.
  const { fps, low } = normalizeScreenProfileSettings();
  const frameRate = low ? Math.min(fps, 15) : fps;
  return { frameRate: { ideal: frameRate, max: frameRate } };
}

function screenTargetProfile() {
  const { quality, fps, low } = normalizeScreenProfileSettings();
...
function updateStreamActualInfo(profile = null) {
  ...
  const ratioLabel = sourceRatio > 0 ? ` • fonte ${sourceW}×${sourceH}` : "";
  el.textContent = `Alvo ${outW}×${outH} • ${target.fps} FPS${ratioLabel}`;
...
async function applyScreenShareSettings({ silent = false } = {}) {
  ...
  const target = screenTargetProfile();
  // Evitamos reaplicar constraints diretamente no getDisplayMedia durante a transmissão.
  // Em Chromium/Electron isso pode causar frames verdes ao redimensionar/maximizar o vídeo.
  // A captura já nasce com o FPS desejado; alterações em tempo real são feitas no RTCRtpSender.
...
async function toggleScreenShare() {
  ...
  try {
    ...
    await replaceVideoForPeers(track); ensureLocalCard(); ensureLocalScreenCard(); refreshMediaSections(); await applyScreenShareSettings({ silent: true }); await retrackVoicePresence();
    const targetProfile = screenTargetProfile();
    const audioMsg = capturedAudio ? " • áudio da tela ativo" : " • sem áudio da tela (marque Compartilhar áudio na janela do navegador)";
    const safetyMsg = (state.preferences.screen_quality === "1440p" && Number(state.preferences.screen_fps || 30) > targetProfile.fps) ? ` • estabilidade: ${targetProfile.quality} em até ${targetProfile.fps} FPS` : "";
    toast(`Compartilhamento iniciado em ${targetProfile.quality} • ${targetProfile.fps} FPS${audioMsg}${safetyMsg}.`, capturedAudio ? 4200 : 6800);
...
async function boot() {
  ...
  if ("serviceWorker" in navigator && location.protocol === "https:") navigator.serviceWorker.register("./sw.js?v=7.5.0").catch(() => {});
