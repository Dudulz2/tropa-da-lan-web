...
const ui = {
  ...
  serverQuickMenu: $("#serverQuickMenu"), quickInviteBtn: $("#quickInviteBtn"), quickCreateChannelBtn: $("#quickCreateChannelBtn"), quickSettingsBtn: $("#quickSettingsBtn"), quickNicknameBtn: $("#quickNicknameBtn"), quickEventsBtn: $("#quickEventsBtn"), quickNotificationsBtn: $("#quickNotificationsBtn"), quickLeaveServerBtn: $("#quickLeaveServerBtn"),
  ...
  leaveVoiceBtn: $("#leaveVoiceBtn"), muteBtn: $("#muteBtn"), deafenBtn: $("#deafenBtn"), cameraBtn: $("#cameraBtn"), shareBtn: $("#shareBtn"), voiceReconnectBtn: $("#voiceReconnectBtn"), voiceDiagnosticsBtn: $("#voiceDiagnosticsBtn"), voiceMicSelect: $("#voiceMicSelect"), voiceOutputSelect: $("#voiceOutputSelect"),
  ...
  diagnosticsDialog: $("#diagnosticsDialog"), commandPaletteDialog: $("#commandPaletteDialog"), commandPaletteInput: $("#commandPaletteInput"), commandPaletteResults: $("#commandPaletteResults"), mobileBottomNav: $("#mobileBottomNav"), toast: $("#toast")
};
...
async function switchMicrophone(deviceId, { persist = true, announce = true } = {}) {
  ...
  const wasMuted = Boolean(oldTrack && !oldTrack.enabled);
  ...
  const replaceResults = await Promise.allSettled([...state.peers.values()].map(async (peer) => {
    if (!peer.audioSender) return;
    await peer.audioSender.replaceTrack(newTrack);
  }));
  if (replaceResults.some((r) => r.status === "rejected")) console.warn("Alguns peers não aceitaram a troca de microfone", replaceResults);
...
async function loadServerNotificationSetting() {
  ...
  const level = state.serverNotificationLevel || state.preferences.notification_level || "mentions";
  if (ui.quickNotificationsBtn) ui.quickNotificationsBtn.textContent = `🔔 Notificações: ${level === "all" ? "todas" : level === "none" ? "nenhuma" : "menções"}`;
}
...
function channelAdminActions(channel) {
  ...
  edit.addEventListener("click", (event) => {
    event.stopPropagation(); openChannelDialog(channel);
  });
...
async function reorderChannelById(sourceId, targetId) {
  ...
  if (!source || !target || source.type !== target.type || (source.category || "Geral") !== (target.category || "Geral")) return;
  const group = state.channels.filter((c) => c.type === source.type && (c.category || "Geral") === (source.category || "Geral")).sort((a,b) => (a.position-b.position) || a.created_at.localeCompare(b.created_at));
...
function renderChannels() {
  ui.channelList.replaceChildren();
  if (!state.activeServer) return;
  const categories = [...new Set(state.channels.map((c) => (c.category || "Geral").trim() || "Geral"))];
  if (!categories.length) categories.push("Geral");
  categories.sort((a,b) => a.localeCompare(b, "pt-BR"));

  const addHeader = (title) => {
    const cat = makeEl("div", "channel-category v7-category");
    cat.appendChild(makeEl("span", "", title.toUpperCase()));
    if (hasPermission("MANAGE_CHANNELS")) {
      const add = makeEl("button", "", "+"); add.type = "button"; add.title = `Criar canal em ${title}`;
      add.addEventListener("click", () => { openChannelDialog(); if (ui.channelCategoryInput) ui.channelCategoryInput.value = title; }); cat.appendChild(add);
    }
  ...
  const makeRow = (channel) => {
    ...
    const symbol = channel.is_private ? "🔒" : channel.type === "voice" ? "🔊" : "#";
    ...
    if (channel.read_only) tags.appendChild(makeEl("span", "channel-tag", "somente leitura"));
    if (Number(channel.slowmode_seconds || 0) > 0) tags.appendChild(makeEl("span", "channel-tag", `${channel.slowmode_seconds}s`));
    nameWrap.appendChild(tags); btn.append(makeEl("span", "channel-symbol", symbol), nameWrap);
    if (channel.type === "text") btn.addEventListener("click", () => selectTextChannel(channel.id)); else btn.addEventListener("click", () => joinVoice(channel.id));
    row.appendChild(btn);
  ...
  for (const category of categories) {
    const channels = state.channels.filter((c) => ((c.category || "Geral").trim() || "Geral") === category).sort((a,b) => (a.type === b.type ? (a.position-b.position) : (a.type === "text" ? -1 : 1)) || a.created_at.localeCompare(b.created_at));
  ...
  if (!state.channels.length && hasPermission("MANAGE_CHANNELS")) addHeader("Geral");
}
...
ui.serverForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (extras.length) {
      const rows = extras.map((c, i) => ({ server_id: serverId, name: c.name, type: c.type, topic: c.topic || "", category: c.category || "Geral", position: 10 + i, created_by: state.user.id, read_only: Boolean(c.read_only) }));
      const { error: templateError } = await state.supabase.from("channels").insert(rows);
...
ui.channelForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (editing) {
      ...
      ({ data, error } = await state.supabase.from("channels").insert({ server_id: state.activeServer.id, type: actualType, position, created_by: state.user.id, ...payload }).select().single());
    }
    if (error) throw error;
    state.editingChannelId = null;
    closeDialog(ui.channelDialog); await loadServerBundle(); await syncVoiceWatchers();
    if (!editing && actualType === "text") await selectTextChannel(data.id);
...
async function joinVoice(channelId) {
  ...
  try {
    ...
    state.localStream = stream; state.voiceJoinedChannelId = channel.id; state.voiceJoinedChannelName = channel.name; if (state.preferences.push_to_talk && micTrack()) micTrack().enabled = false;
...
function ensureLocalCard() {
  ...
  if (!card) {
    ...
    card.append(video, screenPlaceholder, makeEl("span", "media-label"), makeEl("span", "media-connection ok", "você"), controls); ui.mediaGrid.prepend(card);
  }
...
function makeRemoteCard(peerId) {
  ...
  if (!card) {
    ...
    card.append(video, remoteAudio, makeEl("span", "media-label"), makeEl("span", "media-connection", "conectando"), controls);
...
async function toggleScreenShare() {
  ...
  try {
    ...
    const stream = await navigator.mediaDevices.getDisplayMedia({ video: getScreenShareVideoConstraints(), audio: false });
...
async function onSignedIn(user) {
  ...
  try {
    ...
    const msg = String(error?.message || "");
    toast(/push_to_talk|screen_quality|category|is_private|timeout_until|server_notification_settings|member_warnings/i.test(msg)
      ? "Execute MIGRATION_V7_PRO.sql no Supabase para ativar os recursos da V7."
      : /user_preferences|friendships|direct_messages|message_reactions/i.test(msg)
        ? "Execute MIGRATION_V6_ULTIMATE.sql e depois MIGRATION_V7_PRO.sql no Supabase."
        : "A estrutura do banco ainda não está pronta. Confira as migrações V4, V6 e V7.", 9000);
  }
}
