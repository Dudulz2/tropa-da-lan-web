...
const ui = {
  ...
  leaveVoiceBtn: $("#leaveVoiceBtn"), muteBtn: $("#muteBtn"), deafenBtn: $("#deafenBtn"), cameraBtn: $("#cameraBtn"), shareBtn: $("#shareBtn"), voiceMicSelect: $("#voiceMicSelect"),
...
async function refreshMediaDevices({ preserveUi = false } = {}) {
  ...
  try {
    ...
    if (ui.micDeviceStatus) ui.micDeviceStatus.textContent = audio.length ? `${audio.length} microfone(s) detectado(s). Qualidade de voz otimizada.` : "Nenhum microfone detectado.";
...
async function requestLabeledDevices() {
  let temp = null;
  try {
    temp = await navigator.mediaDevices.getUserMedia({ audio: buildVoiceAudioConstraints(ui.audioInputSelect?.value || state.preferences.audio_input_id || ""), video: false });
  } catch (error) {
...
async function switchMicrophone(deviceId, { persist = true, announce = true } = {}) {
  ...
  let stream;
  try {
    stream = await navigator.mediaDevices.getUserMedia({ audio: buildVoiceAudioConstraints(deviceId), video: false });
  } catch (error) {
    if (deviceId && ["OverconstrainedError", "NotFoundError", "DevicesNotFoundError"].includes(error?.name)) {
      stream = await navigator.mediaDevices.getUserMedia({ audio: buildVoiceAudioConstraints(""), video: false });
      deviceId = "";
  ...
  const replaceResults = await Promise.allSettled([...state.peers.values()].map(async (peer) => {
    ...
    await tuneAudioSender(peer.audioSender);
  }));
...
async function testMicrophone() {
  ...
  try {
    ...
    const stream = await navigator.mediaDevices.getUserMedia({ audio: buildVoiceAudioConstraints(deviceId), video: false });
...
async function joinVoice(channelId) {
  ...
  try {
    try {
      stream = await navigator.mediaDevices.getUserMedia({ audio: buildVoiceAudioConstraints(state.preferences.audio_input_id || ""), video: false });
    } catch (error) {
      if (state.preferences.audio_input_id && ["OverconstrainedError", "NotFoundError", "DevicesNotFoundError"].includes(error?.name)) {
        stream = await navigator.mediaDevices.getUserMedia({ audio: buildVoiceAudioConstraints(""), video: false });
        await persistAudioInputPreference("");
        toast("O microfone salvo não está disponível. Usando o microfone padrão.", 4500);
      } else throw error;
    }
    setSpeechHint(stream.getAudioTracks()[0]);
...
function createPeer(peerId) {
  ...
  preferOpus(audioTx); tuneAudioSender(peer.audioSender).catch(() => {});
