...
async function leaveVoice() {
  ...
  state.voiceJoinedChannelId = null; state.voiceJoinedChannelName = null; state.localStream = null; state.cameraTrack = null; state.screenTrack = null; state.deafen = false; state.hardMuted = false; state.cameraBusy = false; state.screenBusy = false;
...
function updateVoiceDock(mode, channel = null) {
  ...
  ui.muteBtn.classList.toggle("active", Boolean(state.hardMuted || (!state.preferences.push_to_talk && micTrack() && !micTrack().enabled)));
...
async function boot() {
  ...
  if ("serviceWorker" in navigator && location.protocol === "https:") navigator.serviceWorker.register("./sw.js?v=7.0.6").catch(() => {});
