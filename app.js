...
function renderChannels() {
  ...
  const collapsedKey = (title) => `tropa_category_collapsed_${state.activeServerId}_${title}`;
  const isCollapsed = (title) => safeLocalStorageGet(collapsedKey(title)) === "1";
  const addHeader = (title, count) => {
    ...
    toggle.append(makeEl("span", "category-chevron", collapsed ? "›" : "⌄"), makeEl("span", "", title.toUpperCase()), makeEl("small", "category-count", String(count || 0)));
    toggle.title = collapsed ? `Expandir ${title}` : `Recolher ${title}`;
    toggle.addEventListener("click", () => { safeLocalStorageSet(collapsedKey(title), collapsed ? "0" : "1"); renderChannels(); });
    cat.appendChild(toggle);
    if (hasPermission("MANAGE_CHANNELS")) {
      const add = makeEl("button", "category-add-button", "+"); add.type = "button"; add.title = `Criar canal em ${title}`;
      add.addEventListener("click", (event) => { event.stopPropagation(); openChannelDialog(); if (ui.channelCategoryInput) ui.channelCategoryInput.value = title; }); cat.appendChild(add);
    }
  ...
  for (const category of categories) {
    ...
    addHeader(category, channels.length);
    if (isCollapsed(category)) continue;
    for (const channel of channels) ui.channelList.appendChild(makeRow(channel));
  }
  if (!state.channels.length && hasPermission("MANAGE_CHANNELS")) addHeader("Geral", 0);
}
...
async function boot() {
  ...
  if ("serviceWorker" in navigator && location.protocol === "https:") navigator.serviceWorker.register("./sw.js?v=7.1.0").catch(() => {});
...
window.TROPA_RUNTIME = {
  ...
  joinVoice, leaveVoice, switchMicrophone, refreshMediaDevices, acquireMicrophone, micTrack, outgoingAudioTrack,
