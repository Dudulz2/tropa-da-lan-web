...
function dedupeVoicePresenceEntries(entries = [], { uniqueUsers = false } = {}) {
  ...
  for (const raw of entries || []) {
    ...
    const currentJoined = Date.parse(current.joined_at || 0) || 0;
    const nextJoined = Date.parse(raw.joined_at || 0) || 0;
    const currentWins = currentJoined && nextJoined ? currentJoined <= nextJoined : String(current.voice_client_id) <= String(raw.voice_client_id);
    const preferred = currentWins ? { ...raw, ...current } : { ...current, ...raw };
    preferred.camera = Boolean(current.camera || raw.camera);
    preferred.screen = Boolean(current.screen || raw.screen);
    preferred.muted = Boolean(current.muted || raw.muted);
    preferred.deafened = Boolean(current.deafened || raw.deafened);
    preferred.joined_at = currentWins ? current.joined_at : raw.joined_at;
    preferred.voice_client_id = currentWins ? current.voice_client_id : raw.voice_client_id;
...
function toggleMediaExpanded(force = null) {
  if (document.body.classList.contains("screen-full-viewport-mode")) clearScreenFillMode();
  const shouldExpand = force === null ? !ui.mediaStage.classList.contains("expanded") : Boolean(force);
  if (shouldExpand) { ui.mediaStage.classList.remove("fullpage"); document.body.classList.remove("call-fullpage-mode"); }
...
function toggleMediaFullpage(force = null) {
  ...
  if (shouldFullpage) ui.mediaStage.classList.remove("expanded");
...
async function boot() {
  ...
  if ("serviceWorker" in navigator && location.protocol === "https:") navigator.serviceWorker.register("./sw.js?v=7.3.11").catch(() => {});
